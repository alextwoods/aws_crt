...
RSpec.describe AwsCrt::Cbor::Decoder do
  def cbor64_decode(value)
    described_class.new(Base64.decode64(value)).decode
  end

  def encode_decode(value)
    described_class.new(AwsCrt::Cbor::Encoder.new.add(value).bytes).decode
  end
  ...
  describe "#decode" do
    it "raises when there are extra bytes" do
      expect do
        cbor64_decode("AAA==")
      end.to raise_error(AwsCrt::Cbor::ExtraBytesError)
    end
    ...
    it "raises for undefined reserved" do
      ...
      expect do
        described_class.new(buffer).decode
      end.to raise_error(AwsCrt::Cbor::Error)
    end
    ...
    it "decodes undefined" do
      ...
      expect(described_class.new(buffer).decode).to eq(:undefined)
    end
